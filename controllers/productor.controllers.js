...
ctrlHome.rutaGet = async (req,res)=>{
        ...
        const productor= await Productor.find().populate('userId','nombre_completo');
        res.json(profesor);
...
ctrlHome.rutaPost = async (req,res)=>{
        ...
        const productor= new Productor(body)

        await productor.save();
        res.json({msg: 'productoragregado'})
};
...
ctrlHome.rutaPut = async (req , res)=>{
        ...
        try {
            const productor= await Productor.findByIdAndUpdate(id, body);
...
ctrlHome.rutaLogicalDelete= async (req, res)=>{
        ...
        try {
            ...
            if (!inactivo.productor) {
                return res.json({
                    msg: `El usuario ${id} no existe`
                });
            };

            const usuario = await User.findByIdAndUpdate(id, { productor: false });
